/**
 * ETAP 12.1: Zaproszenia Owner → Manager
 *
 * Owner może zaprosić managera do zarządzania restauracją
 *
 * POST /api/invites/owner-to-manager
 * Body: {
 *   restaurantId: string
 *   email: string
 *   firstName: string
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Permission check: tylko owner lub super_admin
    if (!['owner', 'super_admin'].includes(actorRole)) {
      return NextResponse.json(
        { error: 'Forbidden - only owner can invite managers' },
        { status: 403 },
      )
    }
