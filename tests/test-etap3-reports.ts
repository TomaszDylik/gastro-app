...
async function main() {
  ...
  try {
    ...
    await prisma.timeEntry.deleteMany({
      where: {
        membershipId: {
          in: [managerMembership?.id, employee1Membership?.id, employee2Membership?.id].filter(
            Boolean
          ) as string[],
        },
        clockIn: {
    ...
    console.log(
      `  - Signature log entries: ${Array.isArray(signedReport.signatureLogJson) ? signedReport.signatureLogJson.length : 0}\n`
    )
    ...
    if (signedReport.signedByUserId !== manager.id) {
      throw new Error(
        `Expected signedByUserId to be ${manager.id}, got ${signedReport.signedByUserId}`
      )
    }
    ...
    console.log(
      `  - Signature log entries: ${Array.isArray(unsignedReport.signatureLogJson) ? unsignedReport.signatureLogJson.length : 0}\n`
    )
    ...
    console.log(`  - Signed by: ${manager.name}`)
    console.log(
      `  - Signature log entries: ${Array.isArray(resignedReport.signatureLogJson) ? resignedReport.signatureLogJson.length : 0}`
    )
    ...
    if (finalLog.length !== 3) {
      throw new Error(
        `Expected 3 signature log entries (sign → unsign → sign), got ${finalLog.length}`
      )
    }
